...
WAN panel (and a one-line probe strip) paints compact LTE status from origin-relative `GET` `/api/lte` only: `stick_present`, `iface`, `ip`, `default_route`, `wan_rebroadcast`, `health`. Labels: **Detected** / **Iface** / **IPv4** / **Default via LTE** / **Mode** / **Health**. `stick_present` false → **No modem**. HTTP error or bad JSON → dashes + Health notes the fetch failed (not **No modem**). Do not dual-write `/api/mode` or `ta_wlan_api`. `usb_id` is not shown.
