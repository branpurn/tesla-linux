...
There is one desktop: XFCE on `DISPLAY=:0`, with a logical **1088x832** framebuffer.

- KMS drives HDMI0 with monitor-compatible `video=HDMI-A-1:1920x1080@60D`.
...
| Path | Role |
|---|---|
| `/boot/firmware/current/cmdline.txt` | Ubuntu 26.04 boot cmdline; forces KMS HDMI0 (`HDMI-A-1`) to **1920x1080@60** |
| `/etc/X11/xorg.conf.d/10-tesla-linux-display.conf` | Selects vc4 as the primary `modesetting` GPU; HDMI-1 preferred mode **1920x1080** |
...
| `tesla-linux-desktop.service` | Sets a 1088x832 logical framebuffer, scales it to HDMI-1 at 1920x1080, then starts XFCE as **teslalinux** |
