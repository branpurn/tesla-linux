...
| `/usr/local/sbin/tesla-linux-wlan` | `boot` / `eth-up` / `ap-up` / `ap-down` / `nginx-bind` / `maybe-ap` / `save-wlan` / **`wan-ap`** (`wan-rebroadcast`) / **`wan-off`** / `wan-verify` |
...
`install-tesla-linux.sh --print-packages` includes `hostapd iw dnsmasq rfkill` and `firefox` (Mozilla apt `.deb`, not the Ubuntu snap stub) in addition to the WAVE 0 stack. Bake already installs NetworkManager as the netplan renderer.
