...
- QEMU picker path is ethernet: `usb-net` / `cdc_ether` → static **10.42.1.1/24**, nginx listen on that IPv4 (never 0.0.0.0). `tesla-linux-wlan` must not fail the oneshot when 10.42.1.1 is bound.
