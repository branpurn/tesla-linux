...
- Serial getty `ttyAMA0` / `ttyS0`: drop-in clears `BindsTo=dev-%i.device` so a missing udev device is not DEPEND-fail. Do not wait on wlan. HDMI `getty@tty1` is masked; Xorg :0 owns vt1.
