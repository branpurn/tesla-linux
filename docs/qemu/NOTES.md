...
- Serial getty `ttyAMA0` / `ttyS0`: drop-in clears `BindsTo=dev-%i.device` so a missing udev device is not DEPEND-fail. Do not wait on wlan. HDMI `getty@tty1` is masked. Xorg :0 is vt1. SSH MOTD is tmux.
...
- HDMI / qemu VNC (tty1) is not a login getty. XFCE is dummy `:0` (`Xorg vt1`); USB KBM is grabbed by that Xorg. SSH attaches tmux (`tl`) for the manage-from-SSH MOTD. Serial-getty remains the typed login path.
